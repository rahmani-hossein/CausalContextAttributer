...
class EfficientPromptGenerator:
    ...
    def create_X(self, mode = "1/p featurization", split_by: str = "sentence") -> np.ndarray:
        ...
        for m, (p, sample_text) in enumerate(zip(self.ps, self.sample_prompts)):
            ...
            # A direct approach:
            #   go through partition.parts in order; 
            #   check if that part's substring is "__" in sample_text
            #   but we must look at partition.indices to see where it should be
...
if __name__ == "__main__":
    ...
    X = prompt_gen.create_X(split_by="sentence", mode= "p featurization")
